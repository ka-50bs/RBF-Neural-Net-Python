...
class PipeLine(object):
    ...
    def norming(self):
        self.__x_forward_edited = self.__x_forward_edited / np.reshape(np.sum(self.__x_forward_edited, axis=1), (-1, 1))
        return self
    ...
    def noise_gen(self, noise_level_forward=0, noise_level_backward=0, forward_sigma=0, backward_sigma=0, replicas=0,
                  noise_type='additive'):
        if noise_type == 'additive_fixed':
            sigma_f = forward_sigma
            sigma_b = backward_sigma
        else:
    ...
    def kmean_clustrer(self, n):
        ...
        kmeans = KMeans(n_clusters=n, random_state=0, max_iter=80, verbose=1)
